...
已有高级筛选实现曾将条件组、顶部标题、清空入口、底部新增和确认入口放在同一个滚动区域里，导致条件较多时头部和尾部操作不可见。统一采用三段式布局：顶部固定区域、条件滚动区域、底部固定区域。

## 本次调整

- 更新 `skills/make-app-filter/SKILL.md`，把高级筛选固定面板布局写成强制规则和交付阻断条件。
...
- 2026-06-24 17:46 CST：先运行新增契约测试，确认旧文档因缺少固定布局基线而失败。
